...
# disable secondary stress, e.g. MARY does not have it
...
# Note that w is an English phoneme, v the German one - we substitute here, is this still needed (?)
BAS_German_trans = {'o~':'O~', 'e~':'E~', 'q':'?' , 'Q':'?', '#':'','+':'','-':'','w':'v','D':'d','{':'E'}
...
def BASpron_to_list(pron,word=''):
    ...
    symbol_sets = [BAS_German_set['silence'],BAS_German_set['primary'],BAS_German_set['items']]
...
def importSampa(myid, word_substitution_dict={}, withFreq=True, manual=False, delimiter='\t', presegmented=False):
            ...
            if withFreq: 
                ...
                if presegmented:
                    if len(split) < 2:
                        ...
                        # filter empty phones from presegmented list
                        pron_list_manuel = [elem for elem in split[1:] if elem!='']

                        # use parser to parse joined phoneme list, warn if there are differences
                        pron_list_auto = BASpron_to_list(''.join(split[1:]),word)

                        if pron_list_auto != pron_list_manuel:
                            print("Warning: pron_list_auto != pron_list_manuel:", pron_list_auto, pron_list_manuel)
                            print("pron_list_auto takes precedence:", word, pron_list_auto)

                        pron_list = pron_list_auto
                elif (len(split)==2):
                    if lineerror:
                        print('Last',no_lineerrors,'lines had wrong format')
...
def guessImportFunc(filename):
    ...
    # manual or semi-automatic lexicon as generated by https://github.com/uhh-lt/speech-lex-edit these are presegmented
    elif filename.endswith('output_lexicon.txt'):
        return partial(importSampa, word_substitution_dict=subs_dict, manual=True, withFreq=False, delimiter=' ', presegmented=True)
    # same as above, this is as an extra lexicon file that ships with kaldi-tuda-de
    elif filename.endswith('de_extra_lexicon.txt'):
        return partial(importSampa, word_substitution_dict=subs_dict, manual=True, withFreq=False, delimiter=' ', presegmented=True)
...
if __name__ == '__main__':
    ...
    with open(args.export_dir + 'nonsilence_phones.txt','w') as nonsilence_phones:
        for item in BAS_German_set['items']:
            ...
            nonsilence_phones.write(item + " '" + item + '\n')
