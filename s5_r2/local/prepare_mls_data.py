...
def process(corpus_path, input_directory, output_datadir):
    ...
    print('Loading', corpus_path + 'metainfo.txt')
    with open(corpus_path + 'metainfo.txt') as metainfo_in:
        ...
        for line in itermetainfo_in:
            split = line.split('|')
            # speaker id
            spk = 'mls-' + split[0].lstrip().rstrip()
            # gender
            gndr = split[1].lstrip().rstrip().lower()
    ...
    with open(corpus_path + input_directory + '/transcripts.txt') as corpus_path_in:
        ...
        for line in itercorpus_path_in:
            ...
            filename = "audio/" + meta[0] + '/' + meta[1] + '/' + myid + '.flac'
            ...
            if os.path.isfile(full_file_path):
                ...
                print("Metadata: ", myid, filename, normalized_text, spk)

    print('done loading ' + corpus_path + input_directory + '/transcripts.txt' + '!')
    ...
    with open(output_datadir + 'wav.scp', 'w') as wav_scp, open(output_datadir + 'utt2spk', 'w') as utt2spk, open(output_datadir + 'text', 'w') as text_out:
        for myid in sorted(corpus.keys()):
            ...
            wav_scp.write(fullid + ' ' + wav_scp_template.replace("$filepath", corpus_path + input_directory + '/' + filename) + '\n')
