...
def augment_wav(wav, dur, fg_snr_opts, bg_snr_opts, fg_noise_utts, \
...
def augment_wav_full(wav, fg_snr_opts, bg_snr_opts, fg_noise_utts, \
    bg_noise_utts, noise_wavs, num_opts):
    """
    Augment the file by ignoring the utterance length
    """
    # This section is common to both foreground and background noises
    new_wav = ""
    # noise_dur = 0
    ...
    if len(bg_noise_utts) > 0:
        ...
        for _ in range(0, num):
            noise_utt = random.choice(bg_noise_utts)
            noise = "wav-reverberate " + noise_wavs[noise_utt] + "\" - |"
            snr = random.choice(bg_snr_opts)
            snrs.append(snr)
            start_times.append(0)
            noises.append(noise)
...
def main():
    ...
    for line in wav_scp_file:
        ...
        try:
            dur = reco2dur[utt]
            new_wav = augment_wav(wav, dur, fg_snrs, bg_snrs, fg_noise_utts,
